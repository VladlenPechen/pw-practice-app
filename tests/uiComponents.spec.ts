...
test.describe('For layouts page @block', () => {
    ...
    test('radio buttons', async ({ page }) => {
        ...
        await usingGridEmailForm.getByLabel('Option 1').check({force: true});

        await usingGridEmailForm.getByRole('radio', {name: 'Option 2'}).check({force: true});

        //generic
        const radio2Status = await usingGridEmailForm.getByRole('radio', {name: 'Option 2'}).isChecked();
        await expect(usingGridEmailForm).toHaveScreenshot();
        expect(radio2Status).toBeTruthy();

        await expect(usingGridEmailForm.getByRole('radio', {name: 'Option 1'})).not.toBeChecked();

        await usingGridEmailForm.getByLabel('Option 1').check({force: true});

        expect(await usingGridEmailForm.getByRole('radio', {name: 'Option 2'}).isChecked()).toBeFalsy();
        expect(await usingGridEmailForm.getByRole('radio', {name: 'Option 1'}).isChecked()).toBeTruthy();
    })
})
...
test('checkboxes', async ({ page }) => {
    ...
    await page.getByRole('checkbox', {name: 'Prevent arising of duplicate toast'}).check({force: true});  //if checked, does not uncheck, you need to use uncheck()
...
test('lists and dropdownds', async ({ page }) => {
...
test("web tables", async ({ page }) => {
    ...
    for (let age of ages) {
        ...
        await page.waitForTimeout(1000); // wait for the table to filter
        ...
        for (let row of await ageRows.all()) {
            ...
            if (Number(age) > 100) {
                expect(await page.getByRole('table').textContent()).toContain('No data found'); // if age is more than 100, it should show No data found
            } else {
...
test('datepicker', async ({ page }) => {
        ...
        let date = new Date();
        date.setDate(date.getDate() + 400); // Set the date to tomorrow
